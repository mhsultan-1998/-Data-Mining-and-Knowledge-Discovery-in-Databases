...
def Terms_and_Conditions():
    ''' 
       ...
       (1) Not sharing your code/solution with any student before and after the homework due. For example, sending your 
           ...
           (if your laptop contains your solution or your Dropbox automatically synchronize your solution between your
            home computer and your laptop) to another student to work on this homework will violate this term.
       (2) Not using anyone's code in this homework, build your own solution. For example, using some code segments from
           another student or online resources due to any reason (like too busy recently) will violate this term. 
           Changing other people's code as your solution (such as changing the variable names) will also violate this
           term.
       (3) When discussing with any other student about this homework, only discuss high-level ideas or using 
           pseudo-code. Don't discuss about the solution at the code level. For example, discussing with another student
           about the solution of a function (which needs 5 lines of code to solve), and then working on the solution 
           "independently", however the code of the two solutions are exactly the same, or only with minor differences 
           (like changing variable names) will violate this term.
      ...
      Historical Data: in one year, we ended up finding 25% of the students in the class violating the terms in their 
...
class TicTacToe(BoardGame):
    '''
        TicTacToe game environment: the goal is to provide a platform for two AI players to play the game in turns and
    ...
    def get_valid_moves(self, s):
        '''
           Get a list of available (valid) next moves from a game state of TicTacToe 
            ...
            Hint: you could use np.where() function to find the indices of the elements in an array, where a test 
            condition is true.
        ...
        `nosetests -v test1.py:test_get_valid_moves' 
        in the terminal.  '''
    ...
    def check_game(self,s):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        for i, k in zip(s.b, np.transpose(s.b)):
            if np.sum(i) == 3 or np.sum(i) == -3:
                return (np.sum(i)/3)
            if np.sum(k) == 3 or np.sum(k) == -3:
                return (np.sum(k)/3)
        for i in [s.b.diagonal(), np.flip(s.b, 1).diagonal()]:
            if np.sum(i) == 3 or np.sum(i) == -3:
                return (np.sum(i)/3)
    ...
    ''' TEST: Now you can test the correctness of your code above by typing 
    `nosetests -v test1.py:test_check_game' 
    in the terminal.  '''
...
#-------------------------------------------------------
''' 
    AI Player 1 (Random Player): Let's first implement the simplest player agent (Random Agent) to get familiar with 
    TicTacToe AI.
'''
#-------------------------------------------------------
class RandomPlayer(Player):
    ...
    def choose_a_move(self,g,s):
        '''
            ...
            Input:
                ...
                s: the current state of the game, which is an object of GameState class.
                 (1)s.b represents the current setting of the board, an integer matrix of shape 3 by 3 (TicTacToe) or 8 
                 by 8 (Othello).
                    s.b[i,j] = 0 denotes that the i-th row and j-th column is empty
                    s.b[i,j] = 1 denotes that the i-th row and j-th column is taken by "X" player. 
                    s.b[i,j] = -1 denotes that the i-th row and j-th column is taken by "O" player.
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        k = g.get_valid_moves(s)
        z = np.random.randint(0, len(k))
        r, c = k[z][0], k[z][1]
    ...
    ''' TEST: Now you can test the correctness of your code above by typing 
    `nosetests -v test1.py:test_choose_a_move'
    in the terminal.  '''
    ...
    '''
    #-----------------------------------------------
    ''' DEMO 2: You can also play the Othello game with the AI (RandomPlayer) by typing 
    `python3 demo2.py' 
    in the terminal.  '''
    #-----------------------------------------------
    #-----------------------------------------------
    ''' DEMO 3: You can also play the GO game with the AI (RandomPlayer) by typing 
    `python3 demo3.py' 
    in the terminal.  '''
    #-----------------------------------------------
...
class Node:
    '''
        Search Tree Node. This is a base/general class of search tree node, which can be used later by all search tree
        based methods, such as MiniMax, Monte-Carlo Tree Search.
...
class MMNode(Node):
    '''
        ...
        List of Attributes: 
            ...
            c: column of the move 
            c: a python list of all the children nodes of this node 
            v: the value of the node (-1, 0, or 1). We assume both players are choosing optimal moves, then this value 
               v represents the best score that "X" player can achieve (1: win, 0:tie, -1: loss)
    '''
    def __init__(self,s,p=None,c=None,m=None,v=None):
    ...
    def expand(self,g):
       ...
       |-------------------------------
               ...
               |    v= None               
               |-----------------------
            Hint: you could use g.get_move_state_pairs() function to compute all the next moves and next game states in
            the game.
            Hint: you could solve this problem using 4 lines of code.
            Hint: in Othello game, given a parent node p and its child node c,  p.x and c.x NOT necessarily have 
            opposite sign.  When there is no valid move for one player, that player will give up the move, so in this
            case the p.x and c.x can be the same.
        '''
        #########################################
        ## INSERT YOUR CODE HERE
        for x in g.get_move_state_pairs(self.s):
            j = MMNode(x[1])
            j.m, j.c, j.p, j.s.b, j.s.x = x[0], [], self, x[1].b, x[1].x
            self.c.append(j)                     
        ...
        # expand the node with one level of children nodes 
            ...
            # append the child node to the child list of the current node 

        #########################################

    ''' TEST: Now you can test the correctness of your code above by typing 
    `nosetests -v test1.py:test_expand' 
    in the terminal.  '''
    ...
    def build_tree(self,g):
        '''
        Given a tree node (the current state of the game), build a fully-grown search tree, which includes all the 
        possible future game states in the tree.  
    ...
    ''' TEST: Now you can test the correctness of your code above by typing 
    `nosetests -v test1.py:test_build_tree' 
    in the terminal.  '''
    ...
    def compute_v(self,g):
        '''
            ...
            Here we assume that the whole search-tree is fully grown, but no value on any node has been computed yet 
        ...
        In Child Node A, it is "O" player's turn, so the value of Child Node A is the MINIMUM of all its children 
        nodes' values: min(1,1,0,1) = 0
        ...
        Now the values of all the children nodes of the current node are ready, we can compute the value of the current
        node.
        In the current node, it is "X" player's turn, so the value of the current node is the MAXIMUM of all its 
        children nodes' values: max(0,0,-1,1,0) = 1
    ...
    ''' TEST: Now you can test the correctness of your code above by typing 
    `nosetests -v test1.py:test_compute_v' 
    in the terminal.  '''
...
class MiniMaxPlayer(Player):
    ...
    def choose_optimal_move(self,n):
        '''
            ...
            (3) Choose Next Move: the agent will choose the child node with the largest/smallest value as the next move.
                if the MinMax player is the "X" player in the game, it will choose the largest value among children 
                nodes. 
                if the MinMax player is the "O" player in the game, it will choose the smallest value among children 
                nodes. 
        ...
        ''' TEST: Now you can test the correctness of your code above by typing 
        `nosetests -v test1.py:test_choose_optimal_move' 
        in the terminal.  '''
    ...
    def choose_a_move(self,g,s):
        '''
            ...
            The goal of this agent is to find the optimal (best) move for the current game state.
            (1) Build Tree: we will first build a fully-grown search tree, where the root of the tree is the current 
            game state.
            (2) Compute Node Values: Then we compute the value of each node recursively using MinMax algorithm.
            (3) Choose Next Move: the agent will choose the child node with the largest/smallest value as the next move.
                    if the MinMax player is the "X" player in the game, it will choose the largest value among children 
    ...
    ''' TEST: Now you can test the correctness of your code above by typing 
    `nosetests -v test1.py:test_minmax_choose_a_move' 
    in the terminal.  '''
